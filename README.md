...
Les signatures RSA PKCS#1 v1.5 sont considérées comme anciennes et obsolètes (Duc Alexandre, 2024). Cependant, elles restent largement utilisées dans des protocoles tels que SSH et IPsec (Keegan Ryan, Kaiwen He, George Arnold Sullivan, Nadia Heninger, 2023). Leur usage persiste notamment parce qu'elles signent un message qui est ensuite transmis de manière chiffrée sur le canal, rendant impossible une attaque directe par calcul du PGCD en cas d’erreur dans la signature.
...
Les serveurs SSH sont identifiés par leurs clés publiques. L'échange commence par une négociation des différents algorithmes utilisés par la suite suivie d'un échange de clés Diffie-Hellman. Le serveur s'authentifie en signant le session identifier avec sa clé privée, vérifiée ensuite par le client. Le session identifier contient le secret partagé du D-H, tous les messages échangés (algorithmes, etc.), l'ID du client et l'ID du serveur.
...
À noter qu'un format non standard de signature RSA est utilisé. RFC 2409 impose une variation du format PKCS#1 v1.5, supprimant l'OID du hachage car il est inclus dans la SA, ce qui diminue la partie connue de la signature et augmente légèrement la complexité de l'attaque.
...
### IKEv2
IKEv2 n'est pas compatible avec IKEv1 et introduit certaines modifications notamment le fait que toutes les signatures sont chiffrées et transmises dans le payload AUTH après l'établissement de la SA. L'extensible Authentication Protocol (EAP) permet d'obtenir de manière active une signature de l'hôte distant sans authentifier l'initiateur de la connection. L’authentification par clés cryptographiques peut ne concerner qu'un participant en fonction des modes utilisés.
#### Sécurité et attaques possibles
...
- Valider les signatures effectuées pour vérifier qu'elles ne contiennent pas d'erreurs avant de les envoyer et, dans le cas contraire, en refaire de nouvelles.
